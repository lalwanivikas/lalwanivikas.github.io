...
input.addEventListener('keyup', function() {
  ...
  if (words) {
    var sentences = input.value.split(/[.|!|?]/gm);
    console.log(sentences);
    var count = 0;
    for (var i = 0; i < sentences.length; i++) {
      // only if there is a 'word' in the sentence it will be counted
      // - otherwise ignored (only blank spaces are not taken into account)
      if (/\w/.test(sentences[i])) {
        count++;
      }
    }
  ...
  if (words) {
    ...
    var keywords = {};
    for (var i = 0; i < wordsWithoutStopWords.length; i++) {
      // checking if the word(property) already exists
      // if it does increment the count otherwise set it to one
      if(wordsWithoutStopWords[i] in keywords) {
        keywords[wordsWithoutStopWords[i]] += 1;
      } else {
        keywords[wordsWithoutStopWords[i]] = 1;
      }
    }
